...
function animateCounter(el, end, suffix = '') {
  if (isNaN(parseInt(end))) { el.textContent = end; return; }
...
async function loadProfile() {
  try {
    ...
    statsEl.querySelectorAll('.stat-item').forEach(el => counterObs.observe(el));
    ...
    document.getElementById('contactLinks').innerHTML = `
      ...
      <a class="contact-link-item" href="${data.github}" target="_blank">
        ...
        <div>
          ...
          <div class="contact-link-value">yourusername</div>
        </div>
      </a>
...
async function loadExperience() {
  try {
    ...
    const emojis = ['🏢', '🚀', '💼'];
    document.getElementById('timeline').innerHTML = experiences.map((exp, i) => `
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  observeReveal('.section-header, .about-avatar-wrap, .about-text, .contact-info, .contact-form');
});
